  ...
  <div class="card card-flush" :class="className">
    ...
    <div class="card-header pt-5">
      ...
      <div class="card-title d-flex flex-column">
        <!--begin::Amount-->
        <span class="fs-2hx fw-bold text-dark me-2 lh-1 ls-n2">{{ stats }}</span>
        <!--end::Amount-->
    ...
    <div class="card-body d-flex flex-column justify-content-end pe-0">
      ...
      <div class="symbol-group symbol-hover flex-nowrap">
        <template v-for="(item, i) in profiles" :key="i">
          <div class="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" :title="item.name">
            <img v-if="item.src" alt="Pic" :src="item.src" />
            <span v-else class="symbol-label fw-bold" :class="`bg-${item.state} text-inverse-${item.state}`">
              {{ item.initials }}
            </span>
          </div>
        </template>
