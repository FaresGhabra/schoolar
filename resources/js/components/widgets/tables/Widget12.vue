            ...
            <template v-for="(item, index) in list" :key="index">
              <tr>
                <td>
                  <div class="d-flex align-items-center">
                    ...
                    <div class="d-flex justify-content-start flex-column">
                      <a href="#" class="text-dark fw-bold text-hover-primary mb-1 fs-6">{{ item.agent.name }}</a>
                      <span class="text-muted fw-semobold text-muted d-block fs-7">{{ item.agent.skills }}</span>
                    </div>
                  </div>
                </td>
                ...
                <td>
                  ...
                  <span class="text-muted fw-semobold text-muted d-block fs-7">{{ item.earnings.remarks }}></span>
                </td>
                ...
                <td>
                  ...
                  <span class="text-muted fw-semobold text-muted d-block fs-7">{{ item.comission.remarks }}</span>
                </td>

                <td>
                  <a href="#" class="text-dark fw-bold text-hover-primary d-block mb-1 fs-6">{{ item.company.name }}</a>
                  <span class="text-muted fw-semobold text-muted d-block fs-7">{{ item.company.fields }}</span>
                </td>

                <td>
                  <div class="rating">
                    <template v-for="i in 5" :key="i">
                      <div :class="[item.rating.value >= i && 'checked']" class="rating-label me-2">
                        <i class="bi bi-star-fill fs-5"></i>
                      </div>
                    </template>
                  </div>

                  <span class="text-muted fw-semobold text-muted d-block fs-7 mt-1">{{ item.rating.remarks }}</span>
                </td>
